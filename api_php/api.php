...
if (!file_exists($arquivo)) {
    file_put_contents($arquivo, json_encode([], JSON_PRETTY_PRINT | JSON_UNESCAPES_UNICODE));
}
...
switch ($metodo) {
    ...
    case 'POST':
        ...
        if (!isset($dados["id"]) || !isset($dados["nome"]) || !isset($dados["email"])) {
            ...
            echo json_encode(["erro" => "Dados incompletos."], JSON_UNESCAPES_UNICODE);
            exit;
        }
        ...
        file_put_contents($arquivo, json_encode($usuarios, JSON_PRETTY_PRINT | JSON_UNESCAPES_UNICODE));
